...
export function SortSelect<TSortKey extends string>({ value, options, onChange, className = '' }: SortSelectProps<TSortKey>) {
  return (
    <label className={`flex items-center gap-2 rounded-xl bg-surface-container-high px-4 py-2 text-on-surface transition-colors hover:bg-surface-container-highest ${className}`}>
      ...
      <select
        ...
        className="max-w-full cursor-pointer bg-transparent text-sm font-semibold text-on-surface outline-none"
        aria-label="Sắp xếp"
      >
        {options.map((option) => (
