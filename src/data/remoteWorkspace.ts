...
export async function fetchRemoteWorkspace(userId: string, email: string | null): Promise<PersistedAppState> {
  ...
  const [
    ...
    supabase.from('activities').select('id, trip_id, date, time, title, location, note, type, image, map_url, booking_code, place_id, is_completed, created_at, updated_at').in('trip_id', tripIds),
    ...
    supabase.from('photos').select('id, trip_id, url, album, created_at, updated_at, storage, provider, provider_public_id, taken_on, place, people, tags, item_type, content, activity_id, place_id').in('trip_id', tripIds),
