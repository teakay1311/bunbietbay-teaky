...
import { createWorkspaceBackupV7, prepareWorkspaceBackup } from '../src/utils/workspaceBackup';
...
test('accepts a complete workspace backup v7', () => {
  const backup = createWorkspaceBackupV7({ workspace: EMPTY_PERSISTED_STATE, library: { notebooks: [{ id: 'local', name: 'Cá nhân', type: 'personal' }], places: [{ id: 'place', notebookId: 'local', name: 'Huế', type: 'other', rating: 5, createdAt: timestamp, updatedAt: timestamp }] }, preferences, tripNotificationPreferences: [] });
  assert.equal(prepareWorkspaceBackup(backup).library?.places[0].name, 'Huế');
});

test('rejects orphaned library places in workspace backup v7', () => {
  const backup = createWorkspaceBackupV7({ workspace: EMPTY_PERSISTED_STATE, library: { notebooks: [], places: [{ id: 'place', notebookId: 'missing', name: 'Huế', type: 'other', rating: 5, createdAt: timestamp, updatedAt: timestamp }] }, preferences, tripNotificationPreferences: [] });
...
test('rejects a trip place linked to a missing library place in v7', () => {
  ...
  const backup = createWorkspaceBackupV7({ workspace, library: { notebooks: [], places: [] }, preferences, tripNotificationPreferences: [] });
